...
const AdventureTours = ({state, actions, libraries}) => {

    useEffect( () => {
        actions.source.fetch("/adventuretours")
    }, [])

    const data = state.source.get('/adventuretours');
    ...
    if(data.isReady) {
        ...
        data.items.map( ({id}) => {
            ...
            const singleTour = state.source.adventuretours[id];
            tours.push(singleTour);
        })
    }

    const pageAdevntureTours = state.source.page[194];
    ...
    return ( 
        ...
        {data.isReady ?
        <>           
        {typeof pageAdevntureTours === "undefined" ? <Loading /> : 
        <>
            <ContainerBackgroundTour>
                ...
                <TextContainer>
                    <h1>  Adventure Tours - <span>Guided by Locals</span> </h1>
            ...
            <MainContainerTours>
                ...
                <ToursContainer>
                {
                    ...
                    <Content>
                        <Html2react html={pageAdevntureTours.content.rendered} />
                    </Content>
...
export default connect(AdventureTours);