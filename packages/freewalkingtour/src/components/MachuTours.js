...
const MPTours = ({state, actions, libraries}) => {
    ...
    useEffect( () => {
        actions.source.fetch("/mptours")
    }, [])

    const data = state.source.get('/mptours');
    ...
    if(data.isReady) {
        ...
        data.items.map( ({id}) => {
            ...
            const singleTour = state.source.mptours[id];
            tours.push(singleTour);
        })
    }

    const pageMPTours = state.source.page[181];
    ...
    return ( 
        ...
        <>
            <ContainerBackgroundTour>
                ...
                <TextContainer>
                    <h1>  Machu Picchu - <span>Guided by Locals</span> </h1>
